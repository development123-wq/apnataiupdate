...
const Banner = () => {
  ...
  // ✅ Static banner (no API)
  const staticBanner = {
    image: '/images/banner/banner1.jpg',
    titleSmall: '',
    titleLarge: 'Browse Catalog',
    buttonText: '',
    buttonLink: '', // e.g. 'https://example.com' (optional)
  };
  ...
  useEffect(() => {
    const fetchRentBudgets = async () => {
      try {
        const res = await axios.get('https://techzenondev.com/apnatai/api/for-rent-budget/1/edit');
            ...
            <select value={rentBudget} onChange={(e) => setRentBudget(e.target.value)}>
              {rentBudgets.length > 0 ? (
                rentBudgets.map((b, idx) => {
                  ...
                  const minLabel = Number.isFinite(Number(b.min)) ? Number(b.min).toLocaleString() : b.min;
                  const maxLabel = Number.isFinite(Number(b.max)) ? Number(b.max).toLocaleString() : b.max;
